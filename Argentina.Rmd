...
sep.miles <- function(x){format(x,big.mark=".")}
  ...
  ### Totales generales
   ...
   ### Departamental
      ...
      casos_pronvinciales <- confirmados %>% left_join(descartados, by=c("carga_provincia_nombre"="carga_provincia_nombre",
      ...
      casos_pronvinciales$positividad <- round(casos_pronvinciales$confirmados/(casos_pronvinciales$confirmados + casos_pronvinciales$descartados)*100, 1)
      
      casos_pronvinciales$confirmados <- if_else(is.na(casos_pronvinciales$confirmados), 0, as.double(casos_pronvinciales$confirmados))
      casos_pronvinciales$altas <- if_else(is.na(casos_pronvinciales$altas), 0, as.double(casos_pronvinciales$altas))
      casos_pronvinciales$descartados <- if_else(is.na(casos_pronvinciales$descartados), 0, as.double(casos_pronvinciales$descartados))
      casos_pronvinciales$fallecidos <- if_else(is.na(casos_pronvinciales$fallecidos), 0, as.double(casos_pronvinciales$fallecidos))
      casos_pronvinciales$positividad <- if_else(is.na(casos_pronvinciales$positividad), 0, as.double(casos_pronvinciales$positividad))
...
```{r mapaActivos, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center',  out.height='100%'}
 activos <- casos_pronvinciales %>% group_by(carga_provincia_nombre) %>% 
  ...
  # %>% filter(departamento == "TOTAL GENERAL") %>%  mutate(activos = cantidad-recuperados-fallecidos) %>%  
  ...
  #  activos$provincia <- if_else(activos$provincia=="Tierra del Fuego, Antártida e Islas del Atlántico Sur", "Tierra del Fuego", activos$provincia)
   activos$carga_provincia_nombre <- if_else(activos$carga_provincia_nombre=="CABA", "Ciudad de Buenos Aires", activos$carga_provincia_nombre)
