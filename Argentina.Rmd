...
edades$grupo <- if_else(edades$edad < 10, "0 a 9", 
  ...
  fallecidos_edad <- fallecidos_edad %>% 
    ...
    select(sexo, edad, cantidad) %>% unique()
     ...
     # unique(casos_pronvinciales$clasificacion) 
...
```{r evolArg, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, paged.print=FALSE}
   ...
   evolArg <- argCov %>%
    ...
    #            breaks = date_breaks("2 weeks"))
     ...
     a <- list( x =  m$dia+20, y =if_else(m$tipo=="Fallecidos", m$cantidad+45000, if_else(m$tipo=="Activos", m$cantidad+70000, m$cantidad)),
...
edad_let <- edades %>% left_join(edades_m, by=c("sexo"="genero", "grupo"="grupo")) %>% 
  group_by(grupo) %>% 
