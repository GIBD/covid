...
casos14 <- covidArg %>% filter(FECHA >= today()-14) %>% group_by(PROVINCIA) %>% mutate(casos14 = sum(CASOS)) %>% 
  select(PROVINCIA, casos14) %>% unique()

casos28 <- covidArg %>% filter(FECHA >= today()-28 & FECHA < today()-14) %>% group_by(PROVINCIA) %>% mutate(casos28 = sum(CASOS)) %>% 
...
# casos_pronvinciales <- fread("data/Covid19Casos.csv")
# system.time(saveRDS(casos_pronvinciales, "data/casos_pronvinciales.rds"))
...
edades$grupo <- if_else(edades$edad < 10, "0 a 9", 
  ...
  fallecidos_edad <- fallecidos_edad %>% 
    ...
    select(sexo, edad, cantidad) %>% unique()
     ...
     # total <- data.frame(table(casos_pronvinciales$clasificacion))
     ...
     #                                 "Caso confirmado por criterio clínico-epidemiológico  - No activo (por tiempo de evolución)")) %>% 
     #    group_by(carga_provincia_nombre) %>% mutate(altas = n()) %>% 
     #    select(carga_provincia_nombre, altas) %>% unique()
     ...
     # table(confirmados_lab$carga_provincia_nombre)
     ...
     # 
       # descartados <- casos_pronvinciales %>% filter(carga_provincia_nombre== residencia_provincia_nombre & 
       #                                                clasificacion_resumen=="Descartado" ) %>% 
       #  group_by(carga_provincia_nombre) %>% mutate(descartados = n()) %>% 
       #  select(carga_provincia_nombre, descartados) %>% unique()
     ...
     # 
       ...
       #                                                clasificacion_resumen=="Confirmado" &
       #                                               fallecido=="SI") %>% 
       # group_by(carga_provincia_nombre) %>% mutate(fallecidos = n()) %>% 
       # select(carga_provincia_nombre, fallecidos) %>% unique()
...
```{r mapaActivos, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center',  out.height='100%'}
    # activos <- casos_provincias %>% group_by(carga_provincia_nombre) %>% filter()
...
```{r DiariosTestD, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='100%'}
  p4 <- plot_ly(argCov, x = ~dia, y = ~MuestraD, type = "bar",  text =~MuestraD, textposition = 'outside', name = "Test Negativos") %>% 
