...
```{r let_Edad}
  ...
  let_edad <- edades %>% left_join(edades_m, by=c("sexo"="genero", "grupo"="grupo")) %>% 
   mutate(let = round(total/cantidad*100, 1))
...
```{r positProv, out.width='100%'}
   testProv <- casos_pronvinciales %>% 
            ...
            select(carga_provincia_nombre, fecha_apertura, positividad) 
      
      fig <- plot_ly(testProv, x = ~fecha_apertura, y = ~positividad, 
                    color = ~carga_provincia_nombre, type = "scatter", mode='lines+markers') 
      fig <- layout(fig, title="Evolución de la Tasa de Positividad por Provincia",
