...
```{r evolArg, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, paged.print=FALSE}
   ...
   evolArg <- argCov %>%
     ...
     a <- list( x = m$dia+1, y = m$cantidad+100, text = paste0('<br>', m$tipo, '\n',m$cantidad, '</br>'),
                xref = "x", yref = "y", showarrow = F, ax = 60, ay = -40  )
     
     b <- list(yref = "y", xref = "x", x="2020-03-20", y= 1, text="ASPO",   showarrow = T)
     ...
     fig %>% layout(title="Evolución de COVID-19 en Argentina",
                    ...
                    annotations=a) %>% layout(showlegend = FALSE,
                    annotations=b) 
```
