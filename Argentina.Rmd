...
```{r ev_provFall, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.height='650px', out.width='100%'}
   ...
   tabla_prov_m<- muertosArg %>% 
      group_by(PROVINCIA, FECHA) %>% mutate(muertes=sum(MUERTES, na.rm = T)) %>%
  select(Provincia=PROVINCIA, fecha=FECHA, muertes) %>% unique() 
...
#### Fallecidos (Log)
```{r ev_provFallLog, echo=FALSE, fig.align='center',out.height='650px',message=FALSE, warning=FALSE, out.width='100%'}
    fig <- plot_ly(p, x = ~fecha, y = ~cant, color = ~provincia, type = "scatter", mode='lines+markers') 
...
```{r promProvSem, echo=FALSE, fig.align='center', out.height='650px', message=FALSE, warning=FALSE, out.width='100%' }
  ...
  fig <- plot_ly(prom, x = ~fecha, y = ~prom, color = ~provincia, type = "bar")
  fig <- layout(fig,
...
## Incidencia diaria
Para seleccionar sólo una provincia haga doble click sobre el nombre de la misma en la leyenda
```{r incProvDiaria, echo=FALSE, fig.align='center',out.height='650px', message=FALSE, warning=FALSE, out.width='100%' }
  # unique(tabla_prov$provincia)
 # b <- list( x="2021-04-19", y= 1, yref = "y", xref = "x", ax=0, ay=-690,
 #                text="", textangle = 0, showarrow = T,
 #                arrowhead = 0,  arrowwidth=0.8, borderwidth=0.5, opacity=0.9)

# poblacionArg$provincia <- if_else(poblacionArg$provincia == "Ciudad de Buenos Aires", "CABA", poblacionArg$provincia)
  ...
  fig <- plot_ly(incidProv, x = ~fecha, y = ~IncP, color = ~provincia, alpha = 1,  size = 30, type = 'area') #,"bar" mode='lines+markers') 
  fig <- layout(fig,  
                autosize = F,  width = 800, height=550,# 
...
```{r diasProv, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='100%'}
...
```{r mapIAArg, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
  ...
  ia_prov$provincia <- if_else(ia_prov$provincia=="CABA", "Ciudad de Buenos Aires", ia_prov$provincia)
  highchart() %>%
...
```{r iAProvD}
  ...
  tabiaprov <- ia_prov %>% select(provincia, cant, ia) %>% arrange(desc(ia)) %>% unique()
...
## Evolucion de Letalidad por Grupoos

```{r volLetalArg, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='100%' }

## Semanas Epidemiológicas
semanas <- read_excel("data/argentina.xlsx", "SEPI")
semanas$desde <- as.Date(semanas$desde, "%d/%m/%Y")    
semanas$hasta <- as.Date(semanas$hasta, "%d/%m/%Y")

  let_gr_ev <- edades_d %>% ungroup() %>% 
    left_join(edades_d_f) %>% ungroup() %>% 
    mutate(fall = if_else(is.na(fall), 0, as.double(fall))) %>% 
    # select(semana, grupo, cantidad, fall) %>% 
    mutate(let = round(fall/cantidad*100, 1)) %>% 
    left_join(semanas, by=c("sepi_apertura"= "semana", "anio"="AÑO")) %>% ungroup() %>% 
    group_by(grupo) %>% arrange(desde, grupo) 
  # %>%  select(desde, grupo, let)
  
  # let_gr_ev$semana <- as.integer(let_gr_ev$semana)
 # let_gr <- let_gr_ev %>% ungroup() %>% group_by(desde) %>% 
 #   mutate(prop = round(let/sum(let)*100, 1)) %>%  # round((fall/sum(fall))/(cantidad/sum(cantidad))*100,1)) %>% 
 #   select(desde, grupo, prop) %>% unique()
 
  fig <- plot_ly(let_gr_ev, x = ~semana, y = ~let, color = ~grupo, alpha = 1,  size = 20, type = 'bar')
  fig <- layout(fig,  
                autosize = F,  width = 800, height=550,# 
                title="Evolución de la Letalidad por Grupo Etario",
                xaxis = list(title = "semana"), yaxis = list(title = "Letalidad"))
  fig
```
...
```{r sud_tasa_mort, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, out.width='100%', fig.align='center'}
  muertosArg$PROVINCIA <- if_else(muertosArg$PROVINCIA=="Ciudad de Buenos Aires", "CABA", muertosArg$PROVINCIA)

  p <- muertosArg  %>% group_by(PROVINCIA) %>% mutate(fall = sum(MUERTES, na.rm = T)) %>% 
    select(PROVINCIA, fall) %>% unique() %>% 
    left_join(poblacionArg, by = c("PROVINCIA"="provincia")) %>% 
    mutate(tasa_mor = round(fall/poblacion * 1000000, 0)) %>% arrange(desc(tasa_mor)) %>% unique()
...
muertosArg$PROVINCIA <- if_else(muertosArg$PROVINCIA=="CABA", "Ciudad de Buenos Aires", muertosArg$PROVINCIA)
...
##### Diarios
```{r DiariosTestD, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='100%'}
  muestras <- argCov %>% ungroup() %>% mutate(promTes = round(roll_mean(MuestraD,  n=7, align="right", fill = NA) ,0))

  p4 <- plot_ly(muestras, x = ~dia, y = ~MuestraD, type = "bar",  text =~MuestraD, textposition = 'outside', name = "Test") %>% 
            
        layout(title="Cantidad de Test Diarios Realizados", yaxis = list(title = "Pruebas Diarias"), barmode = "stack")

  # p4
                # marker = list(color =  'rgb(55, 83, 109)'))
   p4 <- p4 %>% add_trace(x = ~dia, y = ~promTes, name = 'Prom. 7d',  type = 'scatter', mode = 'lines', 
                           # mode = "lines",
                           # marker = list(color = 'rgb(55, 83, 109)'), 
...
```{r positProv, out.height='650px', out.width='100%'}
    testProv <- positiv_lab %>% 
            ...
            mutate(positP=round(roll_mean(positividad, n=7, align="right", fill = NA),0) ) %>% 
...
## Curva Epidémica
Considerando la fecha de inicio de síntomas, o en caso de no existir la menor entre las fechas de apertura o diagnóstico se obtienen la siguiente curva de casos a nivel nacional.
```{r iniSintGral, out.width='100%'}
  ...
  confirmados$fis <- if_else(is.na(confirmados$fecha_inicio_sintomas),
                           min(confirmados$fecha_apertura, confirmados$fecha_diagnostico, na.rm=T), 
                           confirmados$fecha_inicio_sintomas)
  confirmados$fis <- as.Date(confirmados$fis, "%d/%m/%Y")
  
 csos_fis <- casos_pronvinciales %>% filter(clasificacion_resumen=="Confirmado") %>% 
  mutate(fis=if_else(is.na(fecha_inicio_sintomas), fecha_apertura, fecha_inicio_sintomas)) %>% 
  group_by(fis) %>% mutate(casos= as.integer(n())) %>% 
  select(fis, casos) %>% unique() %>% ungroup() %>% arrange(fis) %>% 
  mutate(prom = round(roll_mean(casos, n=7, align="right", fill = NA),0) ) %>%
  filter(fis>'2020-02-01' & !is.na(prom))


  #CASOS en azul
  fig <- plot_ly(csos_fis, x = ~fis, y = ~casos, type = 'bar',  name = 'Casos',
               marker = list(color =  'rgb(55, 83, 109)'))
   fig <- fig %>% add_trace(x = ~fis, y = ~prom, name = 'Promedio',  type = 'scatter', mode = 'lines', 
                           # mode = "lines",
                           # marker = list(color = 'rgb(55, 83, 109)'), 
                           line = list(color = 'rgb(55, 83, 109)', width =4))

  fig <- fig %>% layout(title = 'Casos diarios por Fe. Inicio de Síntomas ',
...
```{r}
 fall <- casos_pronvinciales %>%  ungroup() %>% 
      filter(clasificacion_resumen =="Confirmado" & fallecido=="SI") %>% 
      group_by(fecha_fallecimiento) %>% 
      mutate(fall =n()) %>% select(fecha_fallecimiento, fall) %>%
  unique() %>% ungroup() %>% 
      arrange(fecha_fallecimiento) %>%
      mutate(promF = round(roll_mean(fall, n=7, align="right", fill = NA),0)) %>% 
      select(fecha=fecha_fallecimiento, fall, promF) %>% unique()
     
#FALL ROJO
  fig <- plot_ly(fall, x = ~fecha, y = ~fall, type = 'bar',  name = 'Fall.',
               marker = list(color =  'rgba(222,45,38,0.8)'))
   fig <- fig %>% add_trace(x = ~fecha, y = ~promF, name = 'Promedio',  type = 'scatter', mode = 'lines',
                           line = list(color = 'rgba(222,45,38,0.8)', width =4))

  fig <- fig %>% layout(title = 'Fallecidos diarios por Fecha real',
                        width = 800, #height=550,
                        xaxis = list(title = "", tickfont = list(size = 14, color = 'rgb(55, 83, 109)')),
                        yaxis = list( title = 'Cantidad', titlefont = list(size = 16, color = 'rgb(55, 83, 109)'),
                                      tickfont = list( size = 14, color = 'rgb(107, 107, 107)')),
                        legend = list(x = 0, y = 1, bgcolor = 'rgba(255, 255, 255, 0)',
                                      bordercolor = 'rgba(255, 255, 255, 0)'))
  fig
...
```{r}
# fall2 <- fall
# fall$fecha <- fall$fecha-15
# 
# pba <- fall
# csos_fis
ambos <- csos_fis %>% left_join(fall, by=c("fis"="fecha"))

 ay <- list( tickfont = list(color = "gray"), overlaying = "y",
              side = "right", title = "fall")
   
fig <- plot_ly(ambos, x = ~fis) %>% 
   add_trace( y = ~prom,  name = 'Casos',  type = 'scatter', mode = 'lines',
               line = list(color =  'rgb(55, 83, 109)', width =4)) %>% 
   add_trace(y = ~promF, name = 'Fallecidos',  type = 'scatter', mode = 'lines',
             line = list(color = 'rgba(222,45,38,0.8)', width =4), 
             yaxis = "y2") %>% 
   layout(uniformtext=list(minsize=12, mode='hide'),  
          title="\n Casos y fallecidos por fecha real de ocurrencia",
          # textposition = 'inside',
          xaxis = list(title = ""),
          yaxis = list(title = "Casos"),
          yaxis2 = list(title = "Fall.", overlaying = "y", side = "right"),
          yaxis2 = list( tickfont = list(color = "gray"), overlaying = "y",
              side = "right", title = "fall"))#, #ay,
...
```{r iniSint, out.height='650px', out.width='100%'}
  tProv <- casos_pronvinciales %>% 
    filter(carga_provincia_nombre== residencia_provincia_nombre & 
             clasificacion_resumen=="Confirmado")
  # tProv$fecha_apertura <- as.Date(tProv$fecha_apertura, "%d/%m/%Y")
  # tProv$fecha_diagnostico <- as.Date(tProv$fecha_diagnostico, "%d/%m/%Y")
  # tProv$fecha_inicio_sintomas <- as.Date(tProv$fecha_inicio_sintomas, "%d/%m/%Y")
  # max(casos_pronvinciales$fecha_inicio_sintomas, na.rm=T)

  tProv$fis <- if_else(!is.na(tProv$fecha_inicio_sintomas),
                       tProv$fecha_inicio_sintomas,
                       tProv$fecha_apertura)
                       # if_else((is.na(tProv$fecha_inicio_sintomas) & is.na(tProv$fecha_diagnostico)), 
                       #        tProv$fecha_apertura,
                       #        if_else( (is.na(tProv$fecha_inicio_sintomas) & is.na(tProv$fecha_apertura)), 
                       #          tProv$fecha_diagnostico,
                       #          if_else((is.na(tProv$fecha_inicio_sintomas) & 
  # !is.na(tProv$fecha_apertura) & !is.na(tProv$fecha_diagnostico )),
                       #                 min(tProv$fecha_apertura, tProv$fecha_diagnostico),
                       #                 tProv$fecha_apertura-3))))
                               
  
  tProv <- tProv %>% group_by(carga_provincia_nombre, fis) %>% 
    mutate(confirmados = n()) %>% 
    select(carga_provincia_nombre, fis, confirmados) %>% unique() %>% ungroup() %>% 
    group_by(carga_provincia_nombre) %>% arrange(fis) %>% 
    mutate(prom = round(roll_mean(confirmados, n=7, align="right", fill = NA),0)) 
  ...
  fig <- plot_ly(tProv, x = ~fis, y = ~confirmados, 
                 color = ~carga_provincia_nombre, type = "bar") 
  fig <- fig %>%  add_trace(y = ~prom, # name = 'Prom.',  
                            type = 'scatter', mode = 'lines',
                            color = ~carga_provincia_nombre, width =4)
  fig <- layout(fig,   autosize = F, width = 800, height=550,# 
                title="Evolución de Casos por Fecha de Inicio de Síntomas",
                xaxis = list(title = "día"), yaxis = list(title = "casos"))
  fig
  
  # er <- tProv %>%  filter(carga_provincia_nombre=="Entre Ríos")
  # write.csv(er, "er.csv")
```
...
* Para seleccionar una sola o mas hacer doble clik sobre el nombre de la/s misma/s en la leyenda
```{r fallFeFall, out.height='650px', out.width='100%'}
...
En base a los indicadores definidos por el Min. de Salud de Nación se establece un semáforo de riesgo para cada provincia.
Los indicadores son:

Razón de Crecimiento: casos acumulados en los últimos 14 días vs casos acumulados en 14 días previos

Incidencia Acumulada: cantidad de casos acumulados en los últimos 14 días cada 100.000 habitantes


El riesgo se define como:

* Riesgo Alto: incidencia acumulada en los últimos 14 días mayor o igual a 150 casos cada 100.000 habitantes y Razón mayor o igual a 1,2

* Riesgo Medio: incidencia acumulada en los últimos 14 días mayor o igual a 150 casos cada 100.000 habitantes o Razón mayor o igual a 1,2

* Riesgo Bajo: incidencia acumulada en los últimos 14 días menor a 150 casos cada 100.000 habitantes y Razón menor a 1,2

```{r mapaIndic, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center',  out.height='100%'}

   indicadores$PROVINCIA <- if_else(indicadores$PROVINCIA=="CABA",  "Ciudad de Buenos Aires", indicadores$PROVINCIA)
  
  indicadores$color <- colorR(indicadores$incid, indicadores$razon)
  
  indicadores$Riesgo <- if_else(indicadores$color=="#009900", "Bajo", if_else(indicadores$color=="#F5F93D", "Medio",  if_else(indicadores$color=="#FF0000", "Alto","")))
  
  highchart(width = 900, height = 900) %>%
    hc_title(text = "<i>COVID-19: Indicadores de Riesgo por Provincia en Argentina</i> - <b>GIBD</b>",
             margin = 5, align = "center",  style = list(color = "#000000", useHTML = TRUE, fontWeight = "bold")) %>% 
    hc_tooltip(followPointer =  FALSE) %>%
    hc_add_series_map(argentina, indicadores, name = "Riesgo",
                      value = "Riesgo", joinBy = c("name", "PROVINCIA"),
                      dataLabels = list(enabled = TRUE, format = '{point.properties.woe-name}')) %>%
    # hc_colorAxis(indicadores$color) %>%
    hc_legend(enabled = FALSE) %>%
    hc_mapNavigation(enabled = TRUE) %>%
    hc_add_theme(hc_theme_ffx()) %>%
    hc_add_annotation(xValue = 0, yValue = 0,  title = list(text = 'Fuente: Ministerios de Salud Provinciales')) %>% 
    hc_chart(borderColor = "#000000", borderRadius = 10, borderWidth = 2)
