...
```{r importarArg, message=FALSE, warning=FALSE, include=FALSE}
 ...
 # capUti <- 1721
  ...
  ### Totales generales
      ...
      names(argCov) <- c("dia", "casosD", "Detectados", "muertesD", "Fallecidos", "Recuperados", "UTI", 
                         "MuestraD", "TotalPruebas","DescEpidemiologia", "DescTest", 
                         "Descartados",  "Importados", "Estrecho", "Comunitaria", "Investigacion", 
                         "InicioSint", "activos_uti",	"letalidad")
   ...
   ### Detalle fallecidos
    ...
    ### Datos abiertos
     # base_hoy <- read.csv("https://sisa.msal.gov.ar/datos/descargas/covid-19/files/Covid19Casos.csv")
     # 
     # base_hoy$ultima_actualizacion
     casos_pronvinciales <- read_csv("data/Covid19Casos.csv")
     casos_pronvinciales2 <- casos_pronvinciales
     ...
     # write.csv(confirmados, "confirmados.csv")
     ...
     casos_pronvinciales$activos <- casos_pronvinciales$confirmados - 
                    casos_pronvinciales$altas - casos_pronvinciales$fallecidos
...
```{r datLetProv, echo=FALSE,message=FALSE, warning=FALSE}
 ...
 # let_prov %>% arrange(desc(letalidad)) %>% kable( col.names =  c("Provincia", "Fallecidos", "Casos Acumulados", "Tasa Letalidad"),format.args = list( big.mark=".", decimal.mark = ","),
 #            align =  c('l', 'c', 'c', 'c') ) %>%  kable_styling()
```
...
```{r iniSint, out.width='100%'}
  confirmados <- casos_pronvinciales2 %>%
  ...
  fig <- plot_ly(tProv, x = ~fis, y = ~casos, 
                      color = ~carga_provincia_nombre, type = "bar") #, mode='lines+markers') 
