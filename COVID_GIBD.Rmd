...
En Sud América se informa el día `r day(min(sudamerica$fecha))` de `r format(min(sudamerica$fecha), "%b")` del 2020 en `r sudamerica %>% filter(fecha==min(sudamerica$fecha)) %>% select(pais)`. A continuación se muestran las fechas en que se detectó por primera vez un infectado:
...
sudamerica %>% 
  ...
  facet_wrap(~pais, scales="free", ncol=4)
```
