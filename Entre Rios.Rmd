...
```{r carga, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
  ...
  covid19ER_Conf <- covid19ER %>% filter(clasificacion_resumen=="Confirmado" & carga_provincia_nombre=="Entre Ríos")
  
  covid19ER_Rec <- covid19ER %>% filter(fallecido=="NO" & clasificacion_resumen=="Confirmado" & 
  ...
  covid19ER_Sosp <- covid19ER %>% filter(clasificacion_resumen=="Sospechoso") %>%
     group_by(residencia_provincia_nombre, residencia_departamento_nombre) %>%
     mutate(sospechosos=n()) %>%
     select(residencia_provincia_nombre, residencia_departamento_nombre, sospechosos) %>% unique()
...
```{r locER, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
  ...
  leaflet() %>% addTiles() %>% 
      ...
      addCircleMarkers(data=localidades_er,  
                       ...
                       radius = ~cant*3, weight = 1, color = "blue", 
...
```{r mapaERAct, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
  ...
  group_by(residencia_provincia_nombre, residencia_departamento_nombre) %>% mutate(conf=n()) %>% 
    ...
    left_join(covid19ER_Rec, by=c("residencia_provincia_nombre"="residencia_provincia_nombre", 
  ...
  activos$activos <- activos$conf-activos$altas
  ...
  highchart() %>%
    ...
    hc_add_series_map(entrerios, activos, name = "Activos", value = "activos",
                      joinBy = c("nam", "residencia_departamento_nombre"),
                      dataLabels = list(enabled = TRUE,
                                        format = '{point.properties.nam}: {point.value}')) %>%
