...
dias <- covid_er %>%  filter(FECHA <= (max(FECHA) - days(8))) %>%
...
pobTot <- sum(poblac_er$poblacion[2:18])

### Base Nación
# covid19ER <- read_csv("data/Covid19Casos.csv")
covid19ER <- read_rds("data/casos_pronvinciales.rds")
...
diario <-  covid_er %>% group_by(FECHA) %>%
  mutate(casosD = sum(CASOS)) %>% dplyr::select(FECHA, casosD) %>% unique()
...
diario <- diario %>% ungroup() %>% arrange(FECHA) %>%
  ...
  dplyr::select(FECHA, casosD, casosP) %>% unique()
...
evolER <- covid_er %>% group_by(DEPARTAMENTO, FECHA) %>% 
  mutate(cantidad= sum(CASOS)) %>% dplyr::select(DEPARTAMENTO, FECHA, cantidad) %>% unique()
...
```{r promERDptos, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='100%' }
  diario %>% filter(!(DEPARTAMENTO %in% c("Otros", "Turistas")) & FECHA >="2021-01-01") %>% 
...
```{r fallD, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, paged.print=FALSE, out.width='100%'}
  ...
  mutate(fallD = n()) %>% select(FECHA=fecha_fallecimiento, fallD) %>% unique()%>% ungroup() %>% 
      arrange(FECHA) %>%
      mutate(promF = round(roll_mean(fallD, n=7, align="right", fill = NA),0)) %>% 
      select(FECHA, fallD, promF) %>% unique()
...
covid_er_hoy <- covid_er %>% filter(!(DEPARTAMENTO %in% c("Otros", "Turistas")) & FECHA==max(FECHA)) %>% group_by(DEPARTAMENTO) %>% 
...
Incprov <- diario %>%
  ...
  ungroup() %>% 
  group_by(FECHA) %>% 
  mutate(DEPARTAMENTO="Entre Ríos",
         cantidad = sum(cantidad)) %>% 
  select(DEPARTAMENTO, FECHA, cantidad) %>%  unique() %>% 
  ungroup() %>% 
  mutate(promD= round(roll_mean(cantidad,  n=7, align="right", fill = 0), 1),
         incid = round(cantidad/pobTot*100000, 0),
         incidP =  round(roll_mean(incid,  n=7, align="right", fill = 0), 1)) %>% 
  arrange(DEPARTAMENTO, FECHA)
# incidP
...
incDep <- diario %>% 
  ...
  mutate(incid = round(cantidad/poblacion*100000, 1),
         incidP =  round(roll_mean(incid,  n=7, align="right", fill = 0), 1)) %>% 
  select(DEPARTAMENTO, FECHA, cantidad, promD, incid, incidP) %>% 
  arrange(DEPARTAMENTO, FECHA)
# names(incDep)
# names(Incprov)
incDep2 <-  rbind.data.frame(Incprov, incDep)
...
incDep <- incDep %>% 
  ...
  mutate(incidP2 =  round(roll_mean(incid,  n=14, align="right", fill = 0), 1)) %>%
  arrange(DEPARTAMENTO, FECHA)
...
incDep %>%
  ...
  select(DEPARTAMENTO, FECHA, casos=cantidad, promD, incid, incidP, incidP2) %>% 
  arrange(DEPARTAMENTO, FECHA) %>% 
  datatable(extensions = 'Buttons', rownames = FALSE,
            colnames = c("Departamento", "Fecha", "Casos del día", "Casos Prom. 7 días",
...
```{r positProv,echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
      posit <- posit %>% filter(fecha_diagnostico>="2020-06-01" & departamento != "SIN ESPECIFICAR")
...
#### Diaria (prom 7d)
```{r tabPosi,echo=FALSE, message=FALSE, warning=FALSE, out.width='100%'}
posit %>%  group_by(departamento) %>% 
  filter(fecha_diagnostico == max(fecha_diagnostico)-7) %>%
  mutate(casos=sum(confirmados), desc = sum(descartado), posit= round(casos/(casos+desc)*100,0) ) %>% 
  filter(fecha_diagnostico == max(fecha_diagnostico)) %>% 
  select(departamento, fecha_diagnostico, casos, desc, posit) %>% unique() %>% 
  arrange(departamento) %>%
  datatable(extensions = 'Buttons', rownames = FALSE,
            colnames = c("Departamento", "Fecha", "Confirmados", "Descartados", 
                           "% Posit. Promedio"),
            options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
                           lengthMenu = list(c(10,25,50,-1), c(17,25,50,"All"))))
```
...
#### Evolución
```{r evolConf, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

edades_d_f <-  covid19ER_Conf %>%  
    # filter(clasificacion_resumen=="Confirmado" & fallecido=="SI")%>% 
    mutate(anio = if_else(sepi_apertura==53, 2020, year(fecha_apertura))) %>% 
    group_by(anio, sepi_apertura, edad, edad_anos_meses) %>%
    mutate(cantidad=n()) %>%  select(anio, sepi_apertura, edad, edad_anos_meses, cantidad) %>% unique()
  
  edades_d_f$edad <- if_else(edades_d_f$edad_anos_meses=="Meses", 0, edades_d_f$edad)
...
plot_ly(edades_d_f2, x = ~semana, text = ~prop_f, 
        textposition = 'auto',
...
#### Evolución
```{r evolFallE, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
inc14 <- incDep2 %>% filter(FECHA==max(FECHA)) %>% select(DEPARTAMENTO, incidP) %>% unique()
...
tabla <- covider %>% filter(!(DEPARTAMENTO %in% c("Otros", "Turistas"))) %>% 
  ...
  left_join(inc14, by=c("departamento"="DEPARTAMENTO" )) %>% 
   select(departamento, casos, nexo, lab, altas, fallecidos, fallS, descartado, sospechosos, dias, razon, incidP) %>%
  unique() %>% 
  arrange(departamento)

tabla %>% datatable(extensions = 'Buttons', rownames = FALSE,
                    colnames = c("Departamento", "Casos Detectados", "Por Nexo", "Por Lab.", 
                                 "Recuperados", "Fallecidos", "Fall. SISA", "Descartados", "Sospechosos", "Dias Dupl.", "Razón", "Inc. 14 Días"),
                    options = list(dom = 'Blfrtip',
                                   buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
                                   lengthMenu = list(c(10,25,50,-1), c(10,25,50,"All"))))
```
