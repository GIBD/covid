...
```{r carga, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
  ...
  covid19ER_Desc <- covid19ER %>% filter(clasificacion_resumen=="Descartado" & residencia_provincia_nombre=="Entre Ríos" & carga_provincia_nombre=="Entre Ríos") %>%
...
```{r piramideER, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
  ...
  edadER <- edadER %>% group_by(grEdad, sexo) %>% filter(sexo %in% c("M", "F")) %>% 
    mutate(cant = n()) %>%
...
```{r descartados, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
 ...
 plot_ly(sem, x = ~sepi_apertura, y = ~cant, type = "bar", width = 800, name = "Descartados",  color = I("steelblue3"),
          hoverinfo = 'text',
...
fallecidos <- covid_er_altas %>% 
 ...
 tabla <- covider %>% 
  ...
  select(departamento=DEPARTAMENTO, casos=cantidad, altas) %>% 
   ...
   select(departamento, casos, altas, fallecidos, sospechosos, descartados) %>% unique() %>% arrange(departamento)

tabla %>% datatable(extensions = 'Buttons', rownames = FALSE,
                  colnames = c("Departamento", "Casos Detectados", "Recuperados", "Fallecidos", "Sospechosos", "Descartados"),
