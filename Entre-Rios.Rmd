...
```{r locER, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
  ...
  localidades_er <- covid_er %>% left_join(localidades, by=c("DEPARTAMENTO"="DEPARTAMENTO", "LOCALIDAD"="LOCALIDAD")) %>% 
    group_by(Lat, Long) %>% mutate(cant=sum(CASOS)) %>% 
  ...
  leaflet() %>% addTiles() %>% 
      ...
      addCircles(data=localidades_er, lng =  as.double(localidades_er$Long), lat = as.double(localidades_er$Lat),
              label = ~as.character(localidades_er$cant), popup = ~as.character(localidades_er$cant),
              weight = 1, radius = ~sqrt(localidades_er$cant)*250 ) %>% 
  addEasyButton(
...
fallecidos <- covid19ER %>% filter(clasificacion_resumen=="Confirmado" & fallecido=="SI") %>%
 ...
 tabla <- covid19ER %>% filter(clasificacion_resumen=="Confirmado") %>% 
   group_by(residencia_departamento_nombre) %>% mutate(casos=n()) %>% 
  select(departamento=residencia_departamento_nombre, casos) %>% unique() %>% 
  left_join(covid19ER_Rec, by=c("departamento"="residencia_departamento_nombre")) %>% 
  select(departamento, casos, altas) %>% 
