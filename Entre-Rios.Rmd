...
```{r carga, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
  ...
  # casos a hoy
  casosH <- diasH %>% ungroup() %>%  #filter(fecha==max(fecha)) %>% 
  ...
  casosSem <- dias %>% ungroup() %>%  #filter(fecha==max(fecha)) %>% 
  ...
  names(covid19ER) <- c("id_evento_caso", "sexo", "edad", "edad_anos_meses", "residencia_pais_nombre", "residencia_provincia_nombre",
                        "residencia_departamento_nombre", "carga_provincia_nombre", 	"fecha_inicio_sintomas", 	"fecha_apertura",
  ...
  # table(covid19ER$Clasificacion)
  covid19ER_Conf <- covid19ER %>%
    filter(clasificacion_resumen=="Confirmado" & carga_provincia_nombre=="Entre Ríos" & residencia_provincia_nombre=="Entre Ríos")
  
  covid19ER_Nexo <-covid19ER_Conf %>% filter(Clasificacion %in% c("Caso confirmado por criterio clinico-epidemiológico - Activo", 
   ...
   covid19ER_Lab <-covid19ER_Conf %>% filter(!(Clasificacion %in% c("Caso confirmado por criterio clinico-epidemiológico - Activo", 
                      "Caso confirmado por criterio clínico-epidemiologico - Fallecido",
                      "Caso confirmado por criterio clínico-epidemiológico  - No activo (por tiempo de evolución)",
                      "Caso confirmado por criterio clínico - epidemiológico -  Activo internado"))) %>% 
  ...
  # table(covid19ER_Conf$clasificacion)
  
  covid19ER_Rec <- covid19ER %>%  
    filter(carga_provincia_nombre=="Entre Ríos" &
             fallecido=="NO" & clasificacion_resumen=="Confirmado" & 
  ...
  covid19ER_Fall_sisa <- covid19ER %>%
    filter(carga_provincia_nombre=="Entre Ríos" & residencia_provincia_nombre=="Entre Ríos" & clasificacion_resumen=="Confirmado" & fallecido=="SI")

  covid19ER_Fall_S <-covid19ER_Fall_sisa %>% group_by(residencia_departamento_nombre) %>%  mutate(fallS = n()) %>% 
  ...
  covid19ER_Sosp <- covid19ER %>% 
    filter(clasificacion_resumen=="Sospechoso" & carga_provincia_nombre=="Entre Ríos" & residencia_provincia_nombre=="Entre Ríos") %>%
  ...
  covid19ER_Desc <- covid19ER %>% filter(clasificacion_resumen=="Descartado" & 
                                           residencia_provincia_nombre=="Entre Ríos" & carga_provincia_nombre=="Entre Ríos") %>%
     group_by(residencia_provincia_nombre, residencia_departamento_nombre) %>%
     mutate(descartados=n()) 
  
  descartados <-covid19ER_Desc %>% select(provincia=residencia_provincia_nombre, 
                                          departamento=residencia_departamento_nombre, descartados) %>% unique()
  ### Movilidad
  ...
  color <- function(d){ 
     c <- if_else(d==0, '#FFFFFF',
          if_else(d < 100, '#fdd49e', if_else(d < 500, '#fc8d59',
...
```{r diaC, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, paged.print=FALSE, out.width='100%'}
  ...
  fig <- plot_ly(diario, x = ~FECHA, y = ~casosD, type = 'bar', name = 'Casos Diarios',text= ~casosD,  textposition = 'outside',
...
```{r fallD, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, paged.print=FALSE, out.width='100%'}
 ...
 # diario$FECHA <- as.Date(diario$FECHA, "%d/%m/%Y")
  ...
  fig <- plot_ly(FallDiario, x = ~FECHA, y = ~fallD, type = 'bar', name = 'Fall. Diarios',text= ~fallD,  textposition = 'outside',
        marker = list(color = 'rgba(222,45,38,0.8)'), width = 800)
  # fig <- fig %>% add_trace(y = ~casosP, name = 'Casos Promedios', marker = list(color = 'rgb(55, 83, 109)'),
  #                          text= ~casosP,  textposition = 'outside')
...
fall_er <- covid19ER %>% filter(residencia_provincia_nombre=="Entre Ríos" & carga_provincia_nombre=="Entre Ríos") %>%
  group_by(residencia_departamento_nombre, sexo, edad, fecha_fallecimiento) %>% mutate(fall =n()) %>% 
  select(departamento = residencia_departamento_nombre, fecha= fecha_fallecimiento, fallecidos=fall) %>% unique()

 fig <- plot_ly(fall_er, x = ~fecha, y = ~fallecidos, color = ~departamento,   text= ~fallecidos,  textposition = 'outside', type = "bar") #, mode='lines+markers') 
...
```{r evolER, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, paged.print=FALSE, out.width='100%' }
   ...
   diario <- evolER %>% group_by(DEPARTAMENTO) %>% mutate(promD = roll_mean(cantidad, n=7, align="right", fill = NA))
   ...
   fig <- plot_ly(p, x = ~fecha, y = ~cant, color = ~departamento, width = 800, type = "scatter", 
                 mode='lines+markers')
...
```{r promERDpto, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='100%' }
  ...
  fig <- plot_ly(diario, x = ~FECHA, y = ~promD, color = ~DEPARTAMENTO,   text= ~round(promD, 0),  textposition = 'outside', type = "bar") #, mode='lines+markers') 
...
```{r locERHoy, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
  ...
  leaflet(height = 900, width = 900) %>% addTiles() %>% 
    ...
    addMarkers(data=localidades_er_hoy,
               lng = localidades_er_hoy$Long, lat = localidades_er_hoy$Lat,
...
```{r locER, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
  ...
  leaflet( ) %>% #height = 900, width = 900) %>% 
    addTiles() %>% 
      ...
      addPolygons(data=deptos_er, fill=FALSE, weight = 1, color = "#000")  %>% 
      addFullscreenControl()  %>%
    addMarkers(data=localidades_er, 
               # lng =as.double(localidades_er$Long2), lat = as.double(localidades_er$Lat2),
               # lng =as.double(localidades_er$Long), lat =  as.double(localidades_er$Lat),
                lng = localidades_er$Long, lat = localidades_er$Lat,
               clusterOptions = markerClusterOptions(singleMarkerMode= T), clusterId = "pointsCluster") %>%
    addEasyButton( easyButton(states =  list(easyButtonState( 
      stateName="unfrozen-markers", icon="ion-toggle",
      title="Freeze Clusters",
...
```{r mapaIncCiud, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
 palcontinua <- colorNumeric(palette =  "Reds", 
                             domain = porLoc$incidencia, 
                             reverse = FALSE)
 ...
 leaflet() %>% #height = 900, width = 900) %>% 
   ...
   addProviderTiles(providers$OpenStreetMap) %>% 
   addFullscreenControl()  %>%
   addCircleMarkers(data=porLoc, lng = porLoc$Long, lat = porLoc$Lat,
                    radius = 10,
...
## Con Datos abiertos
# fallecidos <- covid19ER %>% filter(clasificacion_resumen=="Confirmado" & fallecido=="SI") %>%
#   group_by(residencia_departamento_nombre) %>% mutate(fallecidos=n()) %>% 
#   select(departamento = residencia_departamento_nombre, fallecidos)
  
fallecidos <- covid_er_altas %>% 
 ...
 tabla <- covider %>% 
  ...
  select(departamento=DEPARTAMENTO, casos=cantidad, altas) %>% 
   ...
   select(departamento, casos, nexo, lab, altas, fallecidos, fallS, descartados, sospechosos, dias) %>% unique() %>% arrange(departamento)

  tabla %>% datatable(extensions = 'Buttons', rownames = FALSE,
                  colnames = c("Departamento", "Casos Detectados", "Por Nexo", "Por Lab.", 
                               "Recuperados", "Fallecidos", "Fall. SISA", "Descartados", "Sospechosos", "Dias Dupl."),
            options = list(dom = 'Blfrtip',
                           buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
                           lengthMenu = list(c(10,25,50,-1),
                                             c(10,25,50,"All"))))
