...
descartados <-covid19ER_Desc %>% 
  select(provincia=residencia_provincia_nombre,
...
## Incidencia diaria
Evolución de la incidencia (Casos cada 100.000 Habitantes) por departamentos.s 
...
incDep <- diario %>% filter(FECHA > "2020-06-14") %>% 
...
# col <- diario %>%  filter(DEPARTAMENTO=="Colón")
# incDep$incidP <- if_else(is.na(incDep$incidP), 0, incDep$incidP)
...
tabla <- covider %>% 
  ...
  select(departamento=DEPARTAMENTO, casos=cantidad, altas) %>% 
   ...
   select(departamento, casos, nexo, lab, altas, fallecidos, fallS, descartados, sospechosos, dias) %>%
