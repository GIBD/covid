...
# covid_er_hoy <- covid_er %>% filter(!(DEPARTAMENTO %in% c("Otros", "Turistas")) & FECHA>="2021-04-22") %>% group_by(DEPARTAMENTO) %>% 
#   mutate(CASOS=sum(CASOS)) %>% 
#   select(DEPARTAMENTO, CASOS) %>% unique()
# 
# incid <- covid_er_hoy %>% left_join(poblac_er, by=c("DEPARTAMENTO"="departamento"))
# incid$incid <- round(incid$CASOS/incid$poblacion*100000,0)
# 
# incid$parents <- "Confirmados"
# plot_ly(data = incid, type= "treemap",
#         values = ~incid,
#         labels= ~DEPARTAMENTO,
#         parents=  ~parents,
...
#   layout(title = paste("Covid19 ER - Distribución de la Incidencia del 22 al 28 de Abril"))
...
A continuación se grafica el cambio en la movilidad de cada departamento tomando como linea de base la movilidad al mes de Febrero. Los datos utilizados son medidos por Facebook.
 ...
 ```{r movilidad, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, out.width='100%'}
  ...
  p<- datos_mov %>%
     filter(NAME_2 =="Colón" & ds>="2020-06-01") %>%
