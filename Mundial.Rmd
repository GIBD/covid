...
```{r importarMund, message=FALSE, warning=FALSE, include=FALSE}
  ...
  ## Dataset Paises mas afectados y Arg
    { mayores <- c('China', 'France', 'US', 'Spain', 'United States of America', 'Italy', 'India', 'Germany', "Brazil", 'Argentina', 'Chile')#, 'Korea, South')
...
La cifra de casos confirmados del nuevo coronavirus SARS-CoV-2es en la actualidad **`r  sep.miles(as.integer(sum(covid19[,ncol(covid19)]))) `** personas.
...
```{r mapaMundoActivos, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
      ...
      mundialHoy <- mundial_ambos %>% 
        group_by(pais) %>%  
        filter(dia==max(dia)) %>% mutate(activos=casos-muertes-recuperados) 
...
```{r grafLetalidadMundo, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, out.width='100%', fig.align='center',  out.height='100%'}
      ...
      letal <- mundial_ambos %>% 
        group_by(pais) %>% 
         ...
         mutate(letalidad = round(muertes/casos * 100, 1)) %>% unique() %>% arrange(desc(letalidad))
...
```{r letalidadMundo, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
  ...
  letalHoy <- mundial_ambos %>%  group_by(pais) %>%  filter(dia==max(dia) & muertes>=100) %>%
   ...
   letalHoy %>% select(pais, fecha, casos, muertes, letalidad) %>% filter(letalidad >0) %>%
     datatable(extensions = 'Buttons', rownames = FALSE,
               colnames = c("País", "Fecha", "Casos",  "Fallecidos", "Tasa Letalidad"),
