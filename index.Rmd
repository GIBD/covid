...
###  {.tabset  .tabset-fade .tabset-pills}
#### Curvas
```{r grafLetalidadMundo, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r dptoAMBA, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  tabla <- amba %>% mutate(casost=sum(cantidad), 
  ...
  leaflet(data = deptos_covid) %>%      
   addTiles(urlTemplate = urlign ) %>% 
    addPolygons( color = "#444444", weight = 1,
                 ...
                 popup = paste(deptos_covid$provincia_, "  ", deptos_covid$departam_1,
...
rm(deptos_covid, deptos, casos_pronvincialesT, casos_pronvinciales)
```
...
La edad media de los pacientes fallecidos es de `r round(mean(muertosArg$edad), 0)` a̱os, y en particular la edad media de los Hombres fallecidos es de `r round(mean(muertosArg$edad[muertosArg$genero=="Hombre"]), 0)` a̱os y en la Mujeres de `r round(mean(muertosArg$edad[muertosArg$genero=="Mujer"]), 0)` a̱os.
