...
```{r grafTray, echo=FALSE, message=FALSE, warning=TRUE, paged.print=FALSE, out.width='100%'}
...
```{r dinamico, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
# # 
# dh <- sudamerica  %>% #filter(pais=="Argentina") %>%
#   select(pais, dia, casos, semanal)
# 
# plot_ly(dh) %>%
#   add_lines(
#     x = ~casos, y = ~abs(semanal),
#     frame = ~fecha, 
#     color = ~factor(pais),
...
# p <- ggplot(data = dh, aes(x = casos, y = semanal, colour=pais) ) +
#        geom_point(aes(frame = dia), size=1) +
#        theme_void() +
#        coord_fixed()
# 
# 
# pg <- ggplotly(p, width=600, height=400) %>%
#                  animation_opts(200, redraw = FALSE, easing = "linear")
# pg
```
...
Datos actualizados al `r max(covidArg$FECHA) `.
...
Actualizado al `r max(covidArg$FECHA)`.
...
covidArg %>% 
  ...
  select(PROVINCIA, FECHA, diario) %>% unique() %>% 
    ...
    labs( x='', y="Cantidad de casos", caption="Datos: Min. Salud Arg.") + 
  ...
  theme(axis.text.x = element_text(colour="grey20", size=7, angle=45, hjust=.5, vjust=.5),
