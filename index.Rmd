...
## Datos mundiales
  ...
  ### Paises mas afectados
  {
    ...
    masAfectados$nuevos <- 0
      for(p in masAfectados$pais){
        for(i in 3:max(masAfectados$dia[masAfectados$pais==p])-1){
          nuevo <- (masAfectados$casos[((masAfectados[,"pais"]==p)&(masAfectados[,"dia"]==i))] - masAfectados$casos[((masAfectados[,"pais"]==p) & (masAfectados[,"dia"]==(i-1) ))])
    ...
    masAfectados_m <- masAfectados_m %>% gather(fecha, casos, 2:max(col(masAfectados_m)), -`Country/Region`) 
 ...
 {   sudamerica <- covid19 %>% 
    ...
    sudamerica_m<-sudamerica_m %>%
     ...
     # sudamerica$pais <- gsub("Brazil", "Brasil", sudamerica$pais)
     # sudamerica_m$pais <- gsub("Brazil", "Brasil", sudamerica_m$pais)
...
Las muertes por la pandemia alcanzan las `r   as.integer(sum(covid19_m[,ncol(covid19_m)])) `, de acuerdo con el recuento de la universidad estadounidense Johns Hopkins, uno de los indicadores más consultados del mundo.


En el siguiente gráfico se puede analizar la velocidad y magnitud que ha tenido la propagación del Covid-19 en diferentes países hasta la fecha indicada. El hecho de que la visión de este sea logarítmica permite analizar en una escala más adecuada cómo ha sido la evolución de la pandemia. Se incluye China.
...
<!-- ## Argentina vs Países más afectados -->
Las curvas de evolución de casos detectados de COVID-19 en Argentina y en los países mas afectados a nivel mundial son:

```{r mundo, echo=FALSE, out.width='75%', fig.align='center'}
  afectados %>%
    ggplot(aes(x=dia.x, y=casos, group=pais, color=pais)) + 
    geom_line(size=1 ) + geom_point()+
    # scale_color_manual(values=c( "#31319c","#3194f7", '#319c31', '#9cce31', '#ffff31', '#ff9c31', '#ff3100', '#f7396b')) +
    # guides(colour = guide_legend(override.aes = list(size=10))) +
    labs(title="COVID-19 en el Mundo", x="días", y="Cantidad de casos", caption="Datos: Universidad Johns Hopkins") + 
    theme_minimal() + labs(color='Países') 
```

Si se excluye China y se comparan estos paises en los primeros `r max(afectados$dia.x[afectados$pais=="Argentina"]) ` días, que son los transcurridos desde el primer caso detectado en Argentina, se obtienen las siguientes curvas: 


```{r mundo20, echo=FALSE, out.width='75%', fig.align='center'}
  ...
  afectados %>%
    ...
    ggplot(aes(x=dia.x, y=casos, group=pais, color=pais)) + 
    geom_line(size=1 ) + geom_point()+
    labs(title="", x="días", y="Cantidad de casos", caption="Datos: Universidad Johns Hopkins") + 
    theme_minimal() + labs(color='Países') 
  
```


Para poner en contexto la situación, en el siguiente gráfico puede verse cómo fueron evolucionando los casos confirmados en estos países una vez que acanzaron los 100 casos

```{r dia100mundo, echo=FALSE, out.width='75%', fig.align='center'}

  afectados100 <- afectados %>%
   ...
   afectados100 %>%
    ggplot(aes(x=dia.x, y=casos, group=pais, color=pais)) + 
    geom_line(size=1 ) + geom_point() +
    labs(title="", x="días transcurridos luego de los 100 casos", y="Cantidad de casos", caption="Datos: Universidad Johns Hopkins") + 
    theme_minimal() + labs(color='Países') 
...
```{r evolArg, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
 ...
 names(argCov) <- c("dia", "casosD", "Detectados", "muertesD", "Fallecidos", "Recuperados", "UTI", "Descartados", "Mujeres", "Hombres", "Importados", "Estrecho", "Comunitaria", "Investigacion")
 ...
 evolArg <- argCov %>%
   ...
   mutate(Confirmados =Detectados - Fallecidos - Recuperados) %>% 
   select(dia, Confirmados, Fallecidos, Recuperados) %>% 
   gather(tipo, cantidad, c(Confirmados, Fallecidos, Recuperados), -dia) 
...
ggplot(tipoCaso,aes(x=dia,y=porc, fill=tipo))+
  ...
  geom_text(aes(label = paste0(round(porc*100,0),"%")), 
...
```{r test, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='100%'}
  ...
  test <- argCov %>% select(dia, Detectados, Descartados) %>%
      mutate(total= Detectados + Descartados)
  test <- test %>% filter(Descartados>=0) %>%  mutate(prop=Detectados/total * 100)
...
Al día de la fecha las incidencias acumuladas por cada 100000 habitantes en la provincias de Argentina son: 

####  {.tabset  .tabset-fade .tabset-pills}
##### Mapa
```{r mapIAArg, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
 poblacionArg <- read_delim("data/poblacionArg.csv", 
  ...
  highchart() %>%
          hc_title(text = "<i>Incidencia Acumulada COVID-19 en Argentina</i> ",
             ...
             style = list(color = "#780000", useHTML = TRUE)) %>% 
                 hc_tooltip(followPointer =  FALSE) %>%
          hc_add_series_map(argentina, ia_prov, name = "IA",
                            value = "ia", joinBy = c("name", "provincia"),
                            dataLabels = list(enabled = TRUE,
                                              format = '{point.properties.woe-name}')) %>%
...
##### Tabla Datos
Como se observa, la provincia con la mayor Incidencia Acumulada al día de hoy es **`r ia_prov[1,1] `** y le siguen **`r ia_prov[2,1] `** y **`r ia_prov[3,1] `**.
```{r tablaIAArg, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, out.width='100%'}
  ia_prov  %>% kable( col.names =  c("Provincia", "Casos", "Población", "IA"),
            align =  c('l', 'c', 'c', 'c', 'c') ) %>% 
      kable_styling()
