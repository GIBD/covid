  ...
  ## Dataset Provincias Argentinas
    {
      ...
      ### CABA
      {
        ...
        caba_c <- read_delim("data/caba_gral.csv",";", 
                          escape_double = FALSE, locale = locale(encoding = "ISO-8859-1"),
                          trim_ws = TRUE)
      ...
      ### TUCUMAN
        tucuman_c <- read_excel("data/minsalud_casosdpto.xlsx", skip = 2)
        tucuman_c <- tucuman_c[-c(nrow(tucuman_c),nrow(tucuman_c)-1),]
        tucuman_c <- tucuman_c %>% select("Departamento", "Casos")
        tucuman_c$provincia <- "Tucumán"
        names(tucuman_c) <- c( "localidad", "cantidad", "provincia")
        
      casos_pronvinciales <- rbind.data.frame(entre_rios_c, rio_negro_c, 
                                              caba_c, bsas_c, stafe_c, 
                                              neuquen_c, cordoba_c,
                                              tucuman_c)
      rm(entre_rios_c, rio_negro_c,caba_c, bsas_c, stafe_c,neuquen_c, cordoba_c, tucuman_c)
    }
...
```{r dptoArg,echo=FALSE, message=FALSE, warning=FALSE }
  ...
  # casos_pronvinciales$departamen <- if_else(casos_pronvinciales$departamen
...
```{r ev_prov, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='100%'}
  ...
  # 
  # p <- ggplot(tabla_prov, aes(x=fecha, y=cant, color=tipo, group=provincia)) +
  #   geom_line()+
  #   facet_grid(~provincia, scale = "free_y")
  # 
  #   fig <- ggplotly(p)
  #   fig
...
```{r datLetProv, echo=FALSE,message=FALSE, warning=FALSE}
 let_prov %>% arrange(desc(letalidad)) %>% kable( col.names =  c("Provincia", "Muertes", "Casos Acumulados", "Tasa Letalidad"),
            align =  c('l', 'c', 'c', 'c') ) %>%  kable_styling()
```
...
```{r test, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='100%'}
  ...
  plot_ly(test, x = ~dia, y = ~prop, type = "bar", name = "Tasa de Positividad",
    ...
    layout(xaxis=list(title=""), yaxis = list(title = "Tasa de Positividad (%)"))
...
```{r propTestD, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='100%'}
  p4 <- plot_ly(test, x = ~dia, y = ~DescTest, type = "bar", name = "Test Negativos") %>% 
        ...
        layout(yaxis = list(title = "Acumulado de Pruebas Realizadas"), barmode = "stack")
 p4
...
```{r locER, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
  ...
  # deptos_er <- deptos_er %>% left_join(covider, by=c("departamen"="DEPARTAMENTO")) %>% 
  #   group_by(departamen) %>% mutate(cantD=n()) %>% select(departamen, cantD, geometry) %>% unique()
  
    leaflet() %>% addTiles() %>% 
      addProviderTiles(providers$OpenStreetMap) %>% 
      addPolygons(data=deptos_er, fill=FALSE, weight = 1)  %>% 
