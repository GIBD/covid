...
sep.miles <- function(x){format(x,big.mark=".")}
  ...
  ## Dataset Sudamérica
    { 
      ...
      sudamerica$nuevos <- if_else(is.na(sudamerica$nuevos)&sudamerica$dia==1, sudamerica$casos, sudamerica$nuevos)
  ...
  ## Dataset Provincias Argentinas
    {
      ...
      ### RIO NEGRO
      {
        ...
        rio_negro_c <- rio_negro_c %>% select(localidad, casos_acumulados)
      ...
      ### BUENOS AIRES
      {
        ...
        bsas_c <- read_delim("data/buenosaires_gral.csv",  ";", escape_double = FALSE, 
      ...
      ### SANTA FE
      {
        stafe_c <- read_delim("data/santafe_gral.csv",  ";", #skip=3,
                          escape_double = FALSE, locale = locale(encoding = "ISO-8859-1"),
                          trim_ws = TRUE)
        # "https://docs.google.com/spreadsheets/d/16-bnsDdmmgtSxdWbVMboIHo5FRuz76DBxsz_BbsEVWA/export?format=csv&id=16-bnsDdmmgtSxdWbVMboIHo5FRuz76DBxsz_BbsEVWA&gid=1214016401", skip = 3)
        ...
        stafe_c$localidad <- gsub("GáLvez", "Gálvez", stafe_c$localidad)
        stafe_c$localidad <- gsub("Roldan", "Roldán", stafe_c$localidad)
        stafe_c$localidad <- gsub("ChabáS", "Chabas", stafe_c$localidad)
      ...
      {## Hoja de Planilla Excel compartida por Mapache
        ...
        neuquen_c <- read_sheet("https://docs.google.com/spreadsheets/d/16-bnsDdmmgtSxdWbVMboIHo5FRuz76DBxsz_BbsEVWA/edit#gid=1711452704", sheet = 4)
        # neuquen_c$Municipio <-
        neuquen_c$Localidad <- gsub("Ã©", "é", neuquen_c$Localidad)
        neuquen_c$Localidad <- gsub("Ã¡", "á", neuquen_c$Localidad)
        neuquen_c$Localidad <- gsub("Ã³", "ó", neuquen_c$Localidad)
        neuquen_c$Localidad <- gsub("Ã","í", neuquen_c$Localidad)
      ...
      ### CHACO
      {
        chaco_c <- read_delim("data/chaco_gral.csv", ";",
                          escape_double = FALSE, locale = locale(encoding = "ISO-8859-1"),
                          trim_ws = TRUE)
        chaco_c <- chaco_c %>% select(Localidad, casos)
        chaco_c$provincia <- "Chaco"
        names(chaco_c) <- c( "localidad", "cantidad", "provincia")
        chaco_c$localidad <- gsub( "Puerto Bermejo", "Puerto Bermejo Nuevo", chaco_c$localidad)
        chaco_c$localidad <- gsub( "Villa Ángela", "Villa Ángela", chaco_c$localidad)
      }
...
```{r muertesg, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, out.width='100%'}
  sudamerica %>% 
    group_by(pais) %>% 
    ggplot(aes(x=dia, y=muertes, color=pais))+
    geom_line( show.legend=FALSE) +
    geom_smooth(se=F, show.legend = F) +                  
    labs(title="", x='día',
...
```{r muertest, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, out.width='100%'}
  ...
  sudamerica %>% 
    group_by(pais) %>% 
    filter(dia==max(dia)) %>% 
    select(pais, muertes) %>% arrange(desc(muertes)) %>%  
   # muertes %>% 
    kable( col.names =  c("País", "# Fallecidos"),
...
plot_ly(edadesHoy, x = ~porcent, y = ~edad, color = ~sexo) %>%
  add_bars(orientation = "h",
        hoverinfo = "y+text+name", text = ~abs(n), 
...
edades <- edades %>% group_by(grupo, genero) %>% mutate(total = sum(n)) %>% 
...
A continuación se muestra la evolución de casos detectados y fallecidos por día para cada una de las provincias argentinas.
<!-- y la cantidad de casos acumulados. -->
...
```{r ev_provFall, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, out.width='100%'}
   ...
   tabla_prov_m<- muertosArg %>% 
    ...
    tabla_prov_m <- tabla_prov_m %>%
      gather(muertes, acumulados, -c(Provincia, fecha))# %>% ungroup()
...
La tasa de Letalidad a nivel nacional a evolucionado como lo muestra la siguiente gráfica.
