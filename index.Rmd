...
La cifra de contagios confirmados del nuevo coronavirus SARS-CoV-2 alcanzó la barrera de un millón, siendo en la actualidad `r  as.integer(sum(covid19[,ncol(covid19)])) ` los casos confirmados.
...
```{r logar, echo=FALSE, out.width='100%', fig.align='center'}
  ...
  fig <- plot_ly(p, x = ~fecha, color = ~pais) %>%
   add_lines(y = ~casos)
...
```{r mundo, echo=FALSE, out.width='100%', fig.align='center'}
  afectados %>%
...
```{r mundo20, echo=FALSE, out.width='100%', fig.align='center'}
...
```{r dia100mundo, echo=FALSE, out.width='100%', fig.align='center'}
...
```{r evolArg, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
 ...
 names(argCov) <- c("dia", "casosD", "Detectados", "muertesD", "Fallecidos", "Recuperados", "UTI", "Descartados")
