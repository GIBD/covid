...
```{r mapaER}
   entrerios_loc <- fromJSON(file="data/localidadesER.geojson")

 ## Detalle casos
  covid_er <- read_delim("data/covid_ER_casos.csv", ";", 
  ...
  covider_loc <- covid_er %>% group_by(LOCALIDAD) %>%
    mutate(cantidad=n()) %>% select(LOCALIDAD, cantidad) %>% unique()
