...
<!-- ```{r casosSup} -->
<!--  superficie <- tabla_prov %>% group_by(provincia) %>%  -->
<!--   filter(fecha == max(fecha) & tipo=="acum") %>% -->
<!--   left_join(poblacionArg) %>%  -->
<!--   mutate(sup = as.double(cant/(superficie)))%>%  -->
<!--   select(provincia, cant, poblacion, superficie, sup) %>%  -->
<!--   arrange(desc(sup)) -->

<!--   superficie %>% select(provincia, cant, poblacion, superficie, sup) %>% arrange(desc(sup)) %>%  -->
<!--     kable( col.names =  c("Provincia", "Casos", "Población","Superficie", "Casos por Km^2"), -->
<!--               align =  c('l', 'c', 'c', 'c','c', 'c') ) %>%  -->
<!--         kable_styling() -->
...
<!--     # hc_tooltip(followPointer =  FALSE) %>% -->
<!--     hc_add_series_map(... = ) -->
<!--     hc_add_event_point(argentina, superficie, name = "IA", value = "sup", joinBy = c("name", "provincia"), dataLabels = list(enabled = TRUE, format = '{point.properties.woe-name}')) -->

<!--     hc_add_series_map(argentina, superficie, name = "IA", value = "sup", joinBy = c("name", "provincia"), -->
<!--                       dataLabels = list(enabled = TRUE, format = '{point.properties.woe-name}')) -->
