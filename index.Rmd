...
```{r importar, message=FALSE, warning=FALSE, include=FALSE}
  ...
  ## Mapas y Poblaciones
  {
    ...
    entrerios <- fromJSON(file="data/entrerios_dpto.geo.json")
    ...
    poblacion <- data.frame( c('Argentina', 'Bolivia', 'Brazil', 'Chile', 'Colombia', 'Ecuador',
       ...
       # poblacion <- data.frame( c('Argentina', 'Bolivia', 'Brazil', 'Chile', 'Colombia', 'Ecuador',  
       #                           'Paraguay',	'Peru',	'Surinam', 'Uruguay',  'Venezuela', "Mexico"), 
       #                         c(44938712, 11383094,	210147125, 19107216, 50372424, 17300000,
       #                           7152703, 33105273,524000, 3529014, 28067000, 126000000)) 
    ...
    # { mayores <- c('China', 'France', 'US', 'Spain', 'Italy',  'Germany', 'Argentina', 'Mexico')
...
```{r mapaER}
   covid_er <- read_delim("data/covid_entreRios.csv", ";", escape_double = FALSE, locale = locale(encoding = "ISO-8859-1"), 
    trim_ws = TRUE)
    
  covider <- covid_er %>% group_by(DEPARTAMENTO) %>% mutate(cantidad=n()) %>% select(DEPARTAMENTO, cantidad) %>% unique()
