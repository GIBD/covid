  ...
  ## Dataset Provincias Argentinas
    {
      ...
      ### RIO NEGRO
      {
        ...
        rio_negro_c <- rio_negro_c %>% select(City, `Casos Confirmados`)
      ...
      neuquen_c <- read_delim("data/neuquen_casos.csv", ";", escape_double = FALSE, 
                              locale = locale(encoding = "ISO-8859-1"),  trim_ws = TRUE)
      neuquen_c <- neuquen_c %>% select(provincia, ciudad, casos)
      names(neuquen_c) <- c("provincia", "localidad", "cantidad")
      
      casos_pronvinciales <- rbind.data.frame(entre_rios_c, rio_negro_c, caba_c, bsas_c, stafe_c, neuquen_c)
      # casos_pronvinciales <- rbind.data.frame(entre_rios_c, rio_negro_c, caba_c, bsas_c, stafe_c)
      rm(entre_rios_c, rio_negro_c, caba_c, bsas_c, stafe_c, neuquen_c)
...
```{r mapLEtArg, echo=FALSE, message=FALSE, warning=FALSE,  fig.align='center', paged.print=FALSE}
  ...
  highchart() %>%
    ...
    hc_mapNavigation(enabled = TRUE) %>%
    hc_add_theme(hc_theme_ffx()) %>% 
    # hc_annotations( list(labels=let_prov$letalidad) ) %>% 
